...
class NaiveBoundsTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_linear_bounds_shape(self, dtype):
    ...
    lb_in = tf.placeholder(dtype=dtype, shape=(batch_size, input_size))
    ub_in = tf.placeholder(dtype=dtype, shape=(batch_size, input_size))

    lb_out, ub_out = naive_bounds.linear_bounds(w, b, lb_in, ub_in)
  ...
  def test_conv2d_bounds_shape(self, dtype):
    ...
    lb_in = tf.placeholder(dtype=dtype, shape=(
        batch_size, input_height, input_width, input_channels))
    ub_in = tf.placeholder(dtype=dtype, shape=(
        batch_size, input_height, input_width, input_channels))

    lb_out, ub_out = naive_bounds.conv2d_bounds(w, b, padding, strides,
                                                lb_in, ub_in)
  ...
  def test_conv2d_bounds(self, dtype, tol):
    ...
    lb_out_lin, ub_out_lin = _materialised_conv_bounds(
        w, b, padding, strides, lb_in, ub_in)
  ...
  def test_conv1d_bounds_shape(self, dtype):
    ...
    lb_out, ub_out = naive_bounds.conv1d_bounds(w, b, padding, strides[0],
                                                lb_in, ub_in)
  ...
  def test_batchnorm_bounds(self, batchnorm_class, dtype, tol, is_training):
    ...
    lb_out, ub_out = naive_bounds.linear_bounds(w, b, lb_in, ub_in)
    lb_out, ub_out = naive_bounds.batchnorm_bounds(
    ...
    lb_out_lin, ub_out_lin = naive_bounds.linear_bounds(wn, bn, lb_in, ub_in)
...
def _materialised_conv_bounds(w, b, padding, strides, lb_in, ub_in):
  ...
  lb_in_flat = flatten(lb_in)
  ub_in_flat = flatten(ub_in)

  # Materialise the convolution as a (sparse) fully connected linear layer.
  input_shape = lb_in.shape[1:].as_list()
  w_lin, b_lin = common.materialise_conv(w, b, input_shape,
                                         padding=padding, strides=strides)
  lb_out_flat, ub_out_flat = naive_bounds.linear_bounds(
      w_lin, b_lin,
      lb_in_flat, ub_in_flat)

  unflatten = snt.BatchReshape(common.conv_output_shape(input_shape,
                                                        w, padding, strides))

  lb_out = unflatten(lb_out_flat)
  ub_out = unflatten(ub_out_flat)
